...
面向嵌入式工程师（惯导 / 机器人 / 航模 / 自动控制）：在 Hex 数据流上**鼠标拖拽框选**即可定义私有协议帧结构，无需写一行解析代码。数据实时映射为物理量、二维曲线、3D 姿态；再通过可视化控件与指令工厂向下位机发送格式化指令，实现真正的双向闭环调试。想让 AI 代劳？内置 AI 助手能**生成组件、执行操作、感知对话**——从协议模板到悬浮桌宠，说出来即发生。

## 特性

**AI 助手（v0.3.6）**

- 对话即创造：十种输出块一键安装——协议模板 / 控制卡片 / 命令库 / 指令工厂协议 / 主题 / 全局样式 / 停靠面板 / 沙箱小部件 / 无边框挂件 / 直接执行动作 / 高权限脚本
- 说出来即发生：打开面板、切布局、连串口等操作类诉求由 AI 直接执行（27 个白名单动作，破坏性动作红标 + 人工确认）
- `window.uartix` API 桥：所有沙箱组件自动获得键盘 / 光标监听、AI 思维链感知、向 AI 提问、自定义右键菜单、窗口控制、挂件互感广播、语音播报、主题订阅
- 无边框形态：透明窗口 + 按住即拖 + 松开吸附停靠——悬浮仪表盘、通知条、桌面宠物（示例玩法，能力完全通用）
- 思维链多段流式展示、安装卡流式守卫、token 经济性提示词架构；沙箱组件实时跟随全局换肤
...
- 帧画布：字节悬停看属性，右键插入 / 删除帧格；协议簇右键导出 JSON 分享包；骨架态「未见有效帧」排查徽标；Hex 数据流搜索高亮、字节属性面板
- 数据表格：虚拟列表、排序筛选、CSV / Excel 导出
- 控制台：Hex / ASCII / 时间戳多视图，收发分色；演示源无设备也可体验全部功能
- 九套主题（海棠默认 / 深蓝 / 浅蓝 / 护眼绿 / 琥珀 / 琉璃…）色卡选择，深浅完整覆盖；中英双语界面深度覆盖全部面板（页签即时切换）
...
- 内置自动更新；四套预设布局工作区 + 自定义布局槽位（另存 / 切换 / 删除）；协议 / 画布 / 命令库 JSON 互传
...
6. 或者直接把需求说给 AI 助手：「帮我定义这个协议」「做一个无边框悬浮温度计」——生成卡片一键安装
...
src/
  features/
    ai/             AI 助手：对话/流式/十种块安装、uartix 注入桥、挂件宿主、扩展库、动作执行
    protocol/       协议模板引擎、拖拽框选、属性面板
    framecanvas/    Hex 帧画布（Canvas 渲染 + 虚拟化 + 归档池）
    controls/       控制画布、控件、命令库、脚本
    console/        控制台、快捷指令栏、指令工厂
    settings/       设置中心、i18n、布局槽位
    serial/         串口/网络会话状态
    table/ plot/ view3d/ hexview/ video/ help/
  i18n/             中英双语（中心键 + tx 就近双语）
  panels/           dockview 面板注册表
  shell/            自绘标题栏、App 外壳
  shared/           图标、通用组件、hex 字节解析
src-tauri/          Rust：串口、TCP/UDP、并行协议解析、校验、AI SSE 流、文件 IO、updater
```

...
- 3D 轨迹面板（plot3d）：**三组独立轨迹**（惯导推算/实际导航/目标等场景），每组各绑 X/Y/Z 三路变量并**独立选显示模式**——point 实时定位（只留最新点零历史内存）/ points 点集 / line 连线（可滑动平均平滑、叠画顶点）；组级还有着色（时间/通道/组色）、渐隐、密度、最大点数、配对方式与容差、备注；组行可拖协议图例字段智能绑定、双击聚焦、右键单组导出/清空；不再有「混合模式」——不同组各选各的模式天然叠加。全局视图：三轴缩放（等比/逐轴）、网格疏密、跟随、自动旋转、键盘飞行、光标缩放（Operator 只读模式下配置锁定，但校准与查看操作仍可进行）；底部时间条支持历史 scrub 与回放联动 seek、组级清空可撤销栈（组配置一步 Ctrl+Z，清空数据不可撤销）。校准能力两套（**采样源=组1**）：**椭球校准**（进校准模式后八象限点云采样，累积上限 20000 点 → 九参数最小二乘拟合：硬磁偏置 offset×3 + 软磁对称校正矩阵 W×6，并给半径变异系数 cv 与残差 RMS 判质量，结果可复制 JSON/C 数组直接进固件；支持残差着色、原始/校正后显示对比与在线补偿预览）与**六面校准**（加计专用：六个静态姿态各静置 2 秒采集，σ 拒绝晃动，解 offset/gain）。拿到原始磁力计/加速度计时用它把歪掉的球校正成正圆。
...
- plot3d({"op":"…"}) 3D 轨迹写操作（需高权限；**gid:"g1"|"g2"|"g3" 选组，缺省 g1**）：op 可选 bind({"gid"?,"axisX"?,"axisY"?,"axisZ"?,"colorCh"?}) 换轴绑定（该组重灌；组1 换绑清空校准）、set({"gid"?,"colorBy"?,"mode"?,"density"?,"fade"?,"pointSize"?,"opacity"?,"showDots"?,"maxPoints"?,"smooth"?,"smoothWin"?,"pairMode"?,"pairTolMs"?,"name"?,"color"?,"notes"?} 或全局 {"axisScale"?,"showGrid"?,"gridDensity"?,"autoRotate"?,"follow"?,"keyFlight"?,"zoomToCursor"?}；旧 style 键仍兼容：line+points|line|points→组 mode+叠画点) 显示设置、clear({"gid"?}) 清空轨迹数据（不可撤销）、undo()/redo() 组配置撤销重做、calib({"calib":"enter"|"exit"|"start"|"stop"|"clear"|"solve6","gRef"?}) 校准（enter/exit 进出椭球校准模式、start/stop 点云采样、clear 清空、solve6 解六面校准；采样源=组1）。用户说"开始校准""清掉校准数据""组2 画实际轨迹""把目标轨迹叠上"时用
...
- api.app.动作名({参数}) → 控制软件本身，返回 Promise<{ok,data?,err?}>。可用动作：
  ...
  · plot3dRead() 3D 轨迹只读快照（P87a 三组化：groups 各组 axes/mode=point|points|line/着色/密度/配对/备注 + view 全局 + 校准模式/采样覆盖/椭球拟合 cv·rms/六面进度）；plot3d({op:"bind"|"set"|"clear"|"undo"|"redo"|"calib", gid?:"g1"|"g2"|"g3", …}) 3D 写操作（需高权限；gid 缺省 g1；组1 三轴为校准采样源；calib 子动作 enter/exit/start/stop/clear/solve6）

...
await api.app.modbus({ op: "poll.start" });`}</pre>
            ...
            {tab === "plot3d" && (
              <>
                <p><b>3D 轨迹面板</b>支持<b>三组独立轨迹</b>（如惯导推算 / 实际导航 / 目标路径）叠加显示：每组各绑 X / Y / Z 通道并独立选显示模式（实时定位 / 点集 / 连线），支持大坐标（经纬度）自动重锚（跨组并集）、双层 LOD 长跑不卡。工具栏「+ 面板」添加。</p>
                <Section title="三组轨迹与显示模式">
                  <ol className="help-ol">
                    <li>左上<b>组托盘</b>三行（G1 / G2 / G3）：每行 = 可见性眼睛、组色点、名称、模式徽标、<b>X / Y / Z 三个绑定下拉</b>（红绿蓝侧条区分轴）、<b>设置齿轮</b>。三轴绑齐该组才开始绘制；未绑齐的行置灰。</li>
                    <li>每组独立选择显示模式（<b>不提供混合模式</b>——不同组各选各的天然叠加）：<code>实时定位</code>=只显示最新点、零历史内存（看当前车位置）；<code>点集</code>=全部历史点不连线（看打点分布）；<code>连线</code>=按时间连线，可加<b>滑动平均平滑</b>（窗口可调）与线上叠画顶点。</li>
                    ...
                    <li>行操作：双击组行 = 相机聚焦该组；行右键 = 设置 / 隐藏 / 聚焦 / 单组导出 CSV / 单组清空（<b>清空不可撤销</b>，有确认）。</li>
                    <li>三轴来自不同帧/不同采样率时，组设置里可切<b>配对模式</b>（插值/最近点）与容差，右下 HUD 有主组配对统计行；三组量级悬殊可切全局<b>逐轴独立缩放</b>（右键「设置」），避免小跨度组被压扁。</li>
                    <li>右上视角托盘（毛玻璃胶囊）：<b>俯视 / 侧视 / 正视 / 等轴</b>四预设、重置视角、聚焦最新点、跟随模式、自动旋转、<b>撤销/重做</b>、椭球校准、<b>清空数据</b>（三组全清，带确认）。左键拖动旋转、右键拖动平移、滚轮缩放。</li>
                    <li><code>跟随模式</code>：视角平滑锁定主组（组1 优先）最新点；<code>自动旋转</code>：展台展示（两者互斥）。右键菜单按「组 / 模式 / 视图 / 测量 / 数据 / 设置」分组；悬停任意点显示所属组与真实坐标，可一键复制。</li>
                  </ol>
                </Section>

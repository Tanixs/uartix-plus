            ...
            {tab === "canvas" && (
              <>
                ...
                <Section title="三种截帧模式与变长载荷">
                  ...
                  <p><b>校验闭环</b>：画布上把某格定义为「和校验(CK1)」时即可选择算法（默认 sum8），<b>保存即启用</b>——覆盖范围自动设为帧首至校验域前，校验不过的帧直接被过滤；字段宽度自动匹配算法产出（sum8=1B、CRC16/SC+AC=2B、CRC32=4B），不一致会被模板校验拦下。CK2 仅是视觉标注位。校验块悬停可看当前算法与覆盖范围，未启用时会明确提示。</p>
                </Section>

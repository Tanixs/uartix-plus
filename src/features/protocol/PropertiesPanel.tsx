    ...
    return (
      <div className="props-panel">
        ...
        <div className="form-hint">
          <HelpHint text="帧识别字段（功能码/帧型码）在字段属性中开启。协议解析链路：帧头（同步字）定位 → 长度域/固定长度/帧尾定界 → 帧识别字段筛选帧型 → 校验域验证。收满一帧后校验识别字段偏移处的固定字节串，匹配才认定本帧型，不匹配静默丢弃；同簇多帧型（如 WIT 0x51~0x5A、匿名 V7 功能码）靠不同识别值区分。" />
          解析链路：帧头定位 → 定界 → 识别筛选 → 校验
          {(() => {
            const discAt: string[] = [];

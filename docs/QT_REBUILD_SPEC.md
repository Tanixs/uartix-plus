...
- **目标窗口尺寸**：默认 1440×900，最小 1100×700，无边框自绘标题栏。允许全屏放大。
...
`t(key)` 机制；中英两份（src/i18n/strings.ts，zh 全量+en 覆盖约 60+ 条 tb.*/st.*/con.*/hx.*/tbl.*）。**深度面板（帧画布/属性面板/控制画布深处/帮助正文）也要求英文，源码英文某些原因没有覆盖完全**
